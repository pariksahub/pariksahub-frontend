...
function EditQuestion() {
  ...
  const fetchQuestion = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/api/questions/id/${id}`);
      const { question, options: questionOptions } = response.data;
      ...
      setSelectedExamIds(question.exam_ids || []);
      setExplanation(question.explanation || '');
      
      // Handle existing image
      if (question.question_image_url) {
        setExistingImageUrl(question.question_image_url);
        const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000';
        setImagePreview(`${baseUrl}/uploads/${question.question_image_url}`);
      }
      ...
      if (questionOptions && questionOptions.length > 0) {
        const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000';
        formattedOptions = questionOptions.map((opt: any) => ({
          option_type: opt.option_type || 'text',
          option_text: opt.option_text || '',
          path_url: opt.path_url || null,
          _id: opt._id || undefined,
          imagePreview: opt.path_url ? `${baseUrl}/uploads/${opt.path_url}` : undefined
        }));
      }
      
      // Ensure at least 4 options for the form
      while (formattedOptions.length < 4) {
        formattedOptions.push({ option_type: 'text', option_text: '', path_url: null });
      }
      ...
      if (question.answer) {
        ...
        // Check if answer is already a letter (A, B, C, D)
        if (/^[A-Z]$/.test(answerText)) {
          setAnswer(answerText);
        } else {
          // Find the index of the matching option
          const matchingIndex = formattedOptions.findIndex(opt => 
            opt.option_text && opt.option_text.trim() === answerText
          );
          if (matchingIndex >= 0) {
            setAnswer(String.fromCharCode(65 + matchingIndex));
          } else {
            // If no match found, try to use the first character if it's a letter
            const firstChar = answerText.charAt(0).toUpperCase();
            if (/^[A-Z]$/.test(firstChar)) {
              setAnswer(firstChar);
            } else {
              // Default to empty if no valid conversion
              setAnswer('');
            }
          }
        }
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setExistingImageUrl(null); // Clear existing image reference
      setError('');
    }
  };
  ...
  const validateForm = () => {
    ...
    if (questionType === 'image' && !questionImage && !existingImageUrl) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add exam_ids as JSON string
      formData.append('exam_ids', JSON.stringify(selectedExamIds));
      
      // Add options with proper handling of images
      ...
      formData.append('options', JSON.stringify(validOptions.map(opt => ({
        ...
        _id: opt._id // Include existing option ID if it exists
      }))));

      // Add option images separately
      ...
      const response = await axiosInstance.put(`/api/questions/update/${id}`, formData, {
        headers: {
