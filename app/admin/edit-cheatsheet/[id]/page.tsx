...
function EditCheatsheet() {
  ...
  const fetchCheatsheet = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/api/cheatsheets/admin/${id}`);
      const data = response.data;
      
      const sortedItems = data.items && data.items.length > 0 
        ? data.items.sort((a: any, b: any) => (a.order || 0) - (b.order || 0)).map((item: any, i: number) => ({
            ...item,
            _id: item._id || Date.now() + i
          }))
        : [{
            _id: Date.now(),
            title: '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axiosInstance.put(`/api/cheatsheets/admin/${id}`, {
        ...formData,
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-sm">Loading cheatsheet...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-10 pb-8">
        ...
        {success && (
          <div className="mb-4 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-md flex items-center gap-2 text-sm">
            ...
            <span>Cheatsheet updated successfully!</span>
          </div>
        )}
