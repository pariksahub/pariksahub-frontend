...
function EditProgrammingTopic() {
  ...
  const fetchTopic = async () => {
    try {
      setFetching(true);
      const response = await axiosInstance.get(`/api/programming-topics/admin/${id}`);
      ...
      setError(error.response?.data?.error || 'Failed to load topic');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axiosInstance.put(`/api/programming-topics/admin/${id}`, formData);
      setSuccess(true);
      setTimeout(() => {
        router.push('/admin/manage-programming-topics');
      }, 1500);
    } catch (error: any) {
      setError(error.response?.data?.error || 'Failed to update programming topic');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0A0E27] text-white p-4 sm:p-6 mt-5">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">Edit Programming Topic</h1>
            <p className="text-gray-400 text-sm mt-1">Update programming topic details</p>
          </div>
        </div>
        ...
        {success && (
          <div className="mb-6 p-4 bg-green-500/10 border border-green-500/50 rounded-xl flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-400">Topic updated successfully. Redirecting...</p>
            </div>
          </div>
        )}
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#6366F1] hover:bg-[#5558E3] rounded-lg transition-colors"
                >
                  Add
                </button>
              </div>
...
export default EditProgrammingTopic;
