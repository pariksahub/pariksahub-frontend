...
const EditExamPattern = () => {
  ...
  const fetchExamPattern = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/api/exam-patterns/${id}`);
      const examPattern = response.data;
      
      // Transform the data to match our form structure
      setFormData({
        exam_name: examPattern.exam_name || '',
        slug: examPattern.slug || '',
        exam_level: examPattern.exam_level || 'All',
        total_questions: examPattern.total_questions || 100,
        total_marks: examPattern.total_marks || 100,
        duration: examPattern.duration || 120,
        sections: examPattern.sections && examPattern.sections.length > 0 
          ? examPattern.sections.map((section: any) => ({
              section_name: section.section_name || '',
              section_code: section.section_code || '',
              questions: section.questions || 25,
              marks_per_question: section.marks_per_question || 1,
              total_marks: section.total_marks || 25,
              duration: section.duration || 30,
              topics: (section.topics || []) as Topic[],
              question_types: section.question_types || ['MCQ']
            }))
          : [{
              section_name: '',
              section_code: '',
        ...
        marking_scheme: {
          correct_answer: examPattern.marking_scheme?.correct_answer || 1,
          wrong_answer: examPattern.marking_scheme?.wrong_answer || 0,
          negative_marking: examPattern.marking_scheme?.negative_marking || false,
          negative_marking_value: examPattern.marking_scheme?.negative_marking_value || 0
        },
        eligibility_criteria: {
          age_limit: {
            min: examPattern.eligibility_criteria?.age_limit?.min || 18,
            max: examPattern.eligibility_criteria?.age_limit?.max || 35
          },
          education_qualification: examPattern.eligibility_criteria?.education_qualification || '',
          nationality: examPattern.eligibility_criteria?.nationality || 'Indian',
          experience_required: examPattern.eligibility_criteria?.experience_required || ''
        },
        exam_frequency: examPattern.exam_frequency || 'Annual',
        application_fee: examPattern.application_fee || 0,
        exam_mode: examPattern.exam_mode || 'Online',
        language_options: examPattern.language_options || ['English', 'Hindi'],
        important_dates: examPattern.important_dates || [],
        preparation_strategy: {
          recommended_study_time: examPattern.preparation_strategy?.recommended_study_time || '',
          important_topics: examPattern.preparation_strategy?.important_topics || [],
          practice_tips: examPattern.preparation_strategy?.practice_tips || [],
          reference_books: examPattern.preparation_strategy?.reference_books || [],
          online_resources: examPattern.preparation_strategy?.online_resources || []
        },
        difficulty_level: examPattern.difficulty_level || 'Medium',
        competition_level: examPattern.competition_level || 'Medium',
        success_rate: examPattern.success_rate || '10-15%',
        average_score: examPattern.average_score || '60-70',
        exam_official_website: examPattern.exam_official_website || '',
        cutoff_marks: {
          general: examPattern.cutoff_marks?.general || '',
          obc: examPattern.cutoff_marks?.obc || '',
          sc: examPattern.cutoff_marks?.sc || '',
          st: examPattern.cutoff_marks?.st || ''
        },
        is_active: examPattern.is_active !== undefined ? examPattern.is_active : true,
        featured: examPattern.featured || false,
        tags: examPattern.tags || [],
        description: examPattern.description || '',
        exam_logo: examPattern.exam_logo || '',
        exam_color: examPattern.exam_color || '#3B82F6'
      });
    } catch (error) {
      setError('Failed to load exam pattern data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axiosInstance.put(`/api/exam-patterns/admin/${id}`, formData);
      setSuccess(true);
      setTimeout(() => {
      ...
      if (error instanceof AxiosError && error.response?.data?.error) {
        ...
        setError('Failed to update exam pattern');
      }
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-lg">Loading exam pattern...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-xl font-bold text-white">Edit Exam Pattern</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {success && (
          <div className="mb-6 bg-green-500/20 border border-green-500/50 text-green-200 px-6 py-4 rounded-lg flex items-center">
            ...
            <span>Exam pattern updated successfully! Redirecting...</span>
          </div>
        )}
